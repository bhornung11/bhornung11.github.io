...
date:   2024-05-12 20:22:34 +0000
categories: Lorenz curve, geospatial analysis, zebra
---
...
Figure 1. The original images (top row). The RGB tranformed images (bottom row).
...
Figure 2. The original images (top row). The CIE94 tranformed images (bottom row).
...
By choosing which pixels receive what amount of the error it is possible introduce a randomness which, on average, approximates the original colours. The dithered results are displayed in Figure 3.
...
Figure 4. Detail of the original image (left panel). The same area in the dithered CIELab transformed image (middel panel). The same dithered CIELab substituted area with borders (right panel).
