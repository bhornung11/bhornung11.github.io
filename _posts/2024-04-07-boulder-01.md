...
* The cardinality of a set is denoted by the operator $ \vert \cdot \vert $. The cardinality measures how many or how much elements are in a set. For example, $\mathcal{A} = \{ x, y\}$, then $ \vert \mathcal{A} \vert  = 2$.
